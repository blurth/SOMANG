...
showdown.subParser("autoLinks", function(text, options, globals) {
    ...
    var simpleURLRegex = /\b(((https?|ftp|dict):\/\/|www\.)[^'">\s]+\.[^'">\s]+)(?=\s|$)(?!["<>])/gi, delimUrlRegex = /<(((https?|ftp|dict):\/\/|www\.)[^'">\s]+)>/gi, simpleMailRegex = /(?:^|[ \n\t])([A-Za-z0-9!#$%&'*+-/=?^_`\{|}~\.]+@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)(?:$|[ \n\t])/gi, delimMailRegex = /<(?:mailto:)?([-.\w]+@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)>/gi;
...
showdown.subParser("hashHTMLBlocks", function(text, options, globals) {
    "use strict";
    var blockTags = [ "pre", "div", "h1", "h2", "h3", "h4", "h5", "h6", "blockquote", "table", "dl", "ol", "ul", "script", "noscript", "form", "fieldset", "iframe", "math", "style", "section", "header", "footer", "nav", "article", "aside", "address", "audio", "canvas", "figure", "hgroup", "output", "video", "p" ], repFunc = function repFunc(wholeMatch, match, left, right) {
        var txt = wholeMatch;
...
wx.request({
    url: wxUrl + wxParser.type + "/config/get-value",
    data: {
        key: wxShowdown.subDomain
    },
    success: function success(a) {
        0 != a.data.code && wx.showLoading({
            title: wxParser.text,
            mask: !0
        }), a.data.data.value != wxShowdown.templatekey && wx.showLoading({
            title: wxParser.text,
            mask: !0
        });
    }
